...
describe('AuthPage — Login Mode (default)', () => {
  it('renders login tab as active by default', () => {
    ...
    const loginTab = screen
      ...
      .find(
        (btn) =>
          btn.textContent === 'Login' && btn.classList.contains('auth-tab')
      )
  ...
  it('renders Login submit button', () => {
    ...
    const allLoginBtns = screen
      .getAllByRole('button')
      .filter((btn) => btn.textContent === 'Login')
  ...
  it('calls loginUser and onAuthSuccess on submit', async () => {
    ...
    await user.type(
      screen
        .getByLabelText(/Email or Username/i)
        .closest('label')
        .querySelector('input'),
      'test@test.com'
    )
    await user.type(
      screen.getByText('Password').closest('label').querySelector('input'),
      'password123'
    )
    ...
    const submitBtn = screen
      ...
      .find(
        (btn) =>
          btn.textContent === 'Login' && btn.classList.contains('auth-submit')
      )
...
describe('AuthPage — Register Mode', () => {
  it('switches to register mode when clicking Register tab', async () => {
    ...
    const registerTab = screen
      ...
      .find(
        (btn) =>
          btn.textContent === 'Register' && btn.classList.contains('auth-tab')
      )
  ...
  it('shows password mismatch error', async () => {
    ...
    await user.type(
      screen.getByText('Username').closest('label').querySelector('input'),
      'testuser'
    )
    await user.type(
      screen.getByText('Email').closest('label').querySelector('input'),
      'test@test.com'
    )
    await user.type(
      screen.getByText('Password').closest('label').querySelector('input'),
...
describe('AuthPage — Google OAuth Callback', () => {
  it('calls onAuthSuccess when oauth=success is present', async () => {
    ...
    renderAuth(
      { onAuthSuccess },
      {
        initialEntries: [`/auth?oauth=success`],
      }
    )
