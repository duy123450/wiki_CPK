...
const twitterLoginCallback = async (req, res) => {
  try {
    ...
    const frontendUrl = envConfig.FRONTEND_URL || 'http://localhost:5173'
    res.redirect(
      `${frontendUrl}/auth?twitterError=1&msg=${encodeURIComponent(error.message)}`
    )
  }
}
...
const discordLoginCallback = async (req, res) => {
  try {
    ...
    const frontendUrl = envConfig.FRONTEND_URL || 'http://localhost:5173'
    res.redirect(
      `${frontendUrl}/auth?discordError=1&msg=${encodeURIComponent(error.message)}`
    )
  }
}
...
const githubLoginCallback = async (req, res) => {
  try {
    ...
    const frontendUrl = envConfig.FRONTEND_URL || 'http://localhost:5173'
    res.redirect(
      `${frontendUrl}/auth?githubError=1&msg=${encodeURIComponent(error.message)}`
    )
  }
}
